...
import { BenchmarkItem, CodeExample } from './types';
import { AgentCompilerSection } from './components/AgentCompilerSection';
import { AnimatedPipeline, highlightLineText } from './components/AnimatedPipeline';
import { SifrLogo } from './components/SifrLogo';
...
export default function App() {
  ...
  const handleCopyInstall = () => {
    navigator.clipboard.writeText("curl -fsSL https://sifr.dev/install | sh");
  ...
  return (
    <div className="min-h-screen bg-zinc-50 relative selection:bg-sky-500/10 selection:text-sky-700 font-sans text-zinc-900 overflow-x-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-1 bg-gradient-to-r from-transparent via-sky-500/50 to-transparent blur-sm" />
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              <SifrLogo className="w-9 h-9 group-hover:scale-105 transition-transform" />
            ...
            <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-zinc-600">
              ...
              <a 
                href="#blog" 
                onClick={(e) => { e.preventDefault(); setToastMessage("The Sifr Engineering Blog is launching with the v1.0.0 milestone. Stay tuned!"); }}
                className="hover:text-zinc-950 transition-colors"
                id="nav-blog"
              >
                Blog
              ...
              <a 
                ...
                onClick={(e) => {
                  ...
                  if (el) {
                    ...
                    setToastMessage("Sifr Docs are currently integrating offline. View the pipeline below.");
                  }
                }}
                className="hover:text-zinc-950 transition-colors"
                id="nav-docs"
          ...
          <div className="flex items-center gap-4">
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="p-2 text-zinc-600 hover:text-zinc-950 hover:bg-zinc-200/50 rounded-lg transition-all"
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 md:py-20 flex flex-col gap-24 relative z-10">
        
        {/* SECTION 1: THE SPLIT HERO */}
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center min-h-[500px]" id="section-hero">
          
          {/* Left Column (The Brand Copy) */}
          <div className="lg:col-span-6 flex flex-col gap-8">

            <div className="flex flex-col gap-4">
              <h1 className="font-display text-5xl md:text-6xl font-extrabold tracking-tight text-zinc-900 leading-none">
                Write Python<br />
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="flex items-center justify-between gap-3 bg-white border border-zinc-200/80 p-1.5 pl-4 rounded-xl shadow-sm hover:border-zinc-300 transition-all flex-1 font-mono text-sm max-w-md h-12">
                <span className="text-zinc-500 select-all overflow-x-auto whitespace-nowrap scrollbar-none pr-2">
                  curl -fsSL https://sifr.dev/install | sh
                </span>
                <button 
                  onClick={handleCopyInstall}
                  className="flex items-center gap-1.5 px-3 py-2 bg-zinc-900 hover:bg-sky-600 hover:text-white text-zinc-300 rounded-lg transition-all shrink-0"
              ...
              <a 
                href="#pipeline"
                className="inline-flex items-center justify-center gap-2 px-5 bg-sky-50 text-sky-700 hover:bg-sky-100 font-medium rounded-xl border border-sky-200/60 transition-colors whitespace-nowrap h-12"
                id="hero-docs-button"
              >
                Read Docs
          ...
          <div className="lg:col-span-6 flex flex-col" id="cli-panel">
            <div className="w-full bg-zinc-900 text-zinc-100 rounded-2xl terminal-shadow overflow-hidden flex flex-col font-mono text-xs border border-zinc-800">
              {/* Terminal Titlebar */}
              <div className="bg-zinc-950 px-4 py-3 flex items-center justify-between border-b border-zinc-800/80">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-400 text-[11px] ml-2 font-sans font-medium">sifr-build-terminal — bash</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-zinc-500 select-none">UTC</span>
                </div>
              </div>
              ...
              <div className="p-5 min-h-[290px] bg-zinc-900 flex flex-col justify-between">
                <div>
                  ...
                  <div className="text-zinc-500 select-none">// Sifr Interactive Compiler Build Visualizer</div>
                  
                  {/* Active Terminal Output */}
                  <div className="mt-4 space-y-1 font-mono text-[11.5px]">
                    {terminalLogs.length === 0 ? (
                      <div className="text-zinc-650 italic">Terminal ready. Run build pipeline.</div>
                    ) : (
                ...
                <div className="mt-6 pt-4 border-t border-zinc-800/60 flex items-center justify-between text-[11px] text-zinc-400 select-none">
                  <div className="flex items-center gap-2">
                    <span className="relative flex h-2 w-2">
                      <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${compilerState === 'done' ? 'bg-emerald-400' : 'bg-sky-400'}`}></span>
                  ...
                  {compilerState === 'done' ? (
                    ...
                    <span className="text-[10px] text-zinc-500 font-sans">Compiling...</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <section className="flex flex-col gap-12" id="features">
          ...
          <div className="bg-white border border-zinc-200/80 rounded-3xl p-8 md:p-12 shadow-sm flex flex-col gap-8 relative overflow-hidden" id="benchmarks">
            ...
            <div className="flex flex-col gap-3 max-w-3xl mx-auto text-center items-center">
              <div className="p-3 bg-sky-50 text-sky-600 rounded-2xl border border-sky-100/85 shadow-2xs mb-1">
              ...
              <div className="flex flex-col items-center gap-5 max-w-2xl mx-auto text-center mt-2">
                ...
                <p className="text-zinc-500 text-xs md:text-sm font-medium leading-relaxed mt-1 italic">
            ...
            <div className="flex justify-center my-1">
              <div className="inline-flex bg-zinc-100 p-1.5 rounded-2xl border border-zinc-200/80 shadow-inner max-w-md w-full sm:w-auto">
                <button 
                  ...
                  className={`flex-1 sm:flex-initial px-6 py-3 text-xs sm:text-sm md:text-base rounded-xl font-bold tracking-tight transition-all duration-300 flex items-center justify-center gap-2 ${
                    benchmarkMetric === 'speed' 
                      ? 'bg-white text-zinc-950 shadow-md ring-1 ring-zinc-200/40 text-sky-600 scale-[1.02]' 
                      : 'text-zinc-500 hover:text-zinc-950 hover:bg-zinc-50/50'
                  }`}
                  id="speed-metric-btn-v2"
                >
                  <Zap className={`w-4 h-4 md:w-5 md:h-5 transition-colors ${benchmarkMetric === 'speed' ? 'text-sky-500' : 'text-zinc-400'}`} />
                  Execution Speed
                </button>
                <button 
                ...
                >
                  <Cpu className={`w-4 h-4 md:w-5 md:h-5 transition-colors ${benchmarkMetric === 'memory' ? 'text-sky-500' : 'text-zinc-400'}`} />
                  Memory Usage
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-zinc-50/50 rounded-2xl p-6 md:p-8 border border-zinc-100">
              ...
              <div className="lg:col-span-7 flex flex-col gap-5 w-full">
                <div className="text-zinc-500 text-xs font-medium font-sans flex items-center justify-between">
                  <span>{benchmarkMetric === 'speed' ? 'Lower represents faster execution time' : 'Lower represents lesser runtime memory'}</span>
                  <span className="font-mono text-zinc-400 bg-zinc-100 px-2 py-0.5 rounded text-[10px]">Comparative Index</span>
                </div>
                    ...
                    return (
                      <div key={row.name} className="flex flex-col gap-1.5">
                        <div className="flex items-center justify-between text-xs font-mono">
                          <span className={`${isSifr ? 'text-sky-600 font-bold text-sm' : 'text-zinc-700'}`}>{row.name}</span>
                          <span className={`font-semibold ${isSifr ? 'text-sky-600 font-bold text-sm' : 'text-zinc-500'}`}>
                            {benchmarkMetric === 'speed' ? row.speedText : row.memoryText}
                          </span>
                        </div>
                        {/* Smooth progress bar */}
                        <div className={`w-full bg-zinc-200/50 ${isSifr ? 'h-4' : 'h-3'} rounded-full overflow-hidden p-0.5 border border-zinc-100`}>
                          <div 
              ...
              <div className="lg:col-span-5 flex flex-col justify-between gap-6 bg-white/80 p-6 md:p-8 rounded-2xl border border-zinc-200/60 shadow-xs h-full">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      <div className="p-1.5 h-fit bg-sky-50 text-sky-600 rounded-lg">
                        <Cpu className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="text-xs font-bold text-zinc-900">No Garbage Collector Latency</div>
                        <div className="text-[11px] text-zinc-500 leading-relaxed animate-none">
                ...
                <div className="pt-4 border-t border-zinc-150">
                  <a 
                    href="https://sifr.dev/blog/compilation-benchmarks"
                    target="_blank"
          ...
          <div className="bg-white border border-zinc-200/80 rounded-3xl p-8 md:p-12 shadow-sm flex flex-col gap-8 relative overflow-hidden" id="card-checked-error-handling">
            ...
            <div className="flex flex-col gap-3 max-w-3xl mx-auto text-center items-center">
              ...
              <p className="text-zinc-650 text-sm md:text-base leading-relaxed text-center">
                ...
                <div className="bg-zinc-950 rounded-2xl border border-zinc-900 p-5 font-mono text-[11.5px] leading-relaxed text-zinc-300 shadow-sm">
                  <div className="flex items-center justify-between border-b border-zinc-850 pb-2.5 mb-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10.5px] text-zinc-550 font-sans ml-1.5">main.sifr</span>
                    </div>
                  </div>
...
def main():
    ...
    # ERROR: unused Result value`
                ...
                <div className="bg-zinc-950 rounded-2xl border border-zinc-900 p-5 font-mono text-xs shadow-sm shadow-red-950/5">
                  <div className="flex items-center gap-1.5 text-zinc-500 border-b border-zinc-850 pb-2.5 mb-3">
...
def main():
    print(add_one(41))`
                ...
                <div className="bg-zinc-950 rounded-2xl border border-zinc-900 p-4 font-mono text-xs shadow-sm shadow-red-950/5">
                  <div className="flex items-center gap-1.5 text-zinc-500 border-b border-zinc-850 pb-2 mb-2">
                    <Terminal className="w-3.5 h-3.5 text-red-400" />
                    <span className="text-[10px] font-sans uppercase tracking-wider font-semibold">Compiler Feedback</span>
                  </div>
...
help: check whether \`x\` is \`None\` before using it as an integer`}
                ...
                <div className="text-xs text-zinc-500 leading-relaxed font-sans px-1 pt-1">
...
def main():
    print(add_one(41))`
        ...
        <section className="bg-white border border-zinc-200/80 rounded-3xl p-8 md:p-12 shadow-sm flex flex-col gap-12" id="pipeline">
          <div className="flex flex-col gap-3 max-w-3xl mx-auto text-center items-center">
            <span className="text-xs font-bold font-mono tracking-wider text-sky-600 uppercase">Architecture Flow</span>
            <h2 className="font-display text-3xl md:text-4xl font-extrabold text-zinc-950">
      ...
      <footer className="bg-zinc-900 text-zinc-400 border-t border-zinc-800 mt-20 relative z-10" id="footer">
        <div className="max-w-7xl mx-auto px-6 py-12 md:py-16 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col gap-2 text-xs">
            ...
            <span className="text-[10px] text-zinc-600 mt-4">&copy; {new Date().getFullYear()} Sifr project.</span>
          </div>
      ...
      {showBenchmarkModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-xl border border-zinc-200">
            {/* Modal Heading */}
            <div className="px-6 py-4 border-b border-zinc-200 flex items-center justify-between bg-zinc-50">
              <div>
                <h3 className="font-display font-bold text-lg text-zinc-950">Raw Benchmarks Breakdown (20 Algorithmic Categories)</h3>
                <p className="text-xs text-zinc-500 font-mono">Comparing Median execution speed in milliseconds (Lower is better)</p>
              </div>
              <button 
                onClick={() => setShowBenchmarkModal(false)}
                className="px-3 py-1.5 text-xs font-bold hover:bg-zinc-200/60 text-zinc-600 rounded-lg transition-all"
            ...
            <div className="px-6 py-3 border-b border-zinc-100 flex items-center gap-2">
              <input 
                ...
                className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-sky-500 font-mono"
              />
              {searchBenchmark && (
                <button 
                  onClick={() => setSearchBenchmark('')}
                  className="text-xs text-zinc-500 hover:text-zinc-950 px-2"
                >
                  Clear
                </button>
              )}
            ...
            <div className="overflow-y-auto px-6 py-4 flex-1">
              <table className="w-full text-left font-mono text-[11px] border-collapse">
                <thead>
                  <tr className="border-b border-zinc-200 text-zinc-400 select-none">
                    ...
                    <th className="py-2.5 font-bold text-right text-sky-600 bg-sky-50 px-2 rounded-t">Sifr</th>
                    ...
                    filteredBenchmarks.map((item) => (
                      <tr key={item.name} className="hover:bg-zinc-50 group">
                        <td className="py-3 pr-4">
                          ...
                          <div className="text-[10px] text-zinc-400 font-sans mt-0.5">{item.description}</div>
                        </td>
                        ...
                        <td className="py-3 text-right text-sky-600 font-bold bg-sky-50/50 px-2 group-hover:bg-sky-50 transition-colors">{item.sifr.toFixed(1)}</td>
                        <td className="py-3 text-right text-orange-600 font-semibold">{item.rust.toFixed(1)}</td>
                        <td className="py-3 text-right text-zinc-400">{item.unit}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
            ...
            {/* Modal Footer */}
            <div className="px-6 py-3 border-t border-zinc-200 bg-zinc-50 flex justify-between items-center text-[10px] text-zinc-500 font-mono">
              <span>Comparing hardware target: AWS c6i.metal (Ice Lake Gen)</span>
              <a 
                href="https://github.com" 
                target="_blank" 
                rel="noreferrer" 
                className="hover:text-sky-600 flex items-center gap-1 font-sans"
              >
                Learn more or contribute tests <ExternalLink className="w-3 h-3" />
              </a>
            </div>
          </div>
        </div>
      ...
      {toastMessage && (
        <div 
          className="fixed bottom-6 right-6 z-50 bg-zinc-900 border border-zinc-800 text-zinc-100 px-4 py-3 rounded-xl shadow-xl flex items-center gap-2.5 animate-bounce-in text-xs font-medium"
          id="system-toast"
        >
